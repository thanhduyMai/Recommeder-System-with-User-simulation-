...
try:
    ...
    print("⚠️ Elasticsearch lib not found. Installing...")
    import subprocess
    subprocess.check_call(["pip", "install", "elasticsearch==7.13.4"])
    from elasticsearch import Elasticsearch
...
from bigdl.dllib.nn.layer import Sequential, Linear, ReLU, ConcatTable, CAddTable, Square, Sum, Sigmoid
...
OFFLINE_METRICS_INDEX = "model_offline_metrics"
...
print(f"🚀 Sending Offline Metrics to Elasticsearch ({ES_HOST})...")
...
try:
    ...
    doc = {
        ...
        "model_name": "DeepFM",
        "metric_type": "offline_eval",
        "hit_rate_k": final_metrics['hit_rate'],
        "ndcg_k": final_metrics['ndcg'],
        "k_value": 10,
        "train_epochs": 20 # Hoặc lấy từ biến config
    }
    
    # Gửi lên index riêng
    res = es.index(index=OFFLINE_METRICS_INDEX, body=doc)
    print(f"✅ Metrics pushed to Kibana! ID: {res['_id']}")
    
except Exception as e:
    print(f"❌ Failed to push metrics to ES: {e}")
